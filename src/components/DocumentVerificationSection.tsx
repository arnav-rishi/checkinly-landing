      ...
      <div className="container-max">
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <div className="space-y-3 sm:space-y-4">
            ...
            return <Card key={step.id} className={`p-4 sm:p-6 cursor-pointer transition-all duration-300 hover:shadow-md border touch-target ${isActive ? 'border-primary/20 bg-primary/5 shadow-md' : 'border-border/50 hover:border-primary/50'}`} onClick={() => setActiveStep(index)}>
                  <div className="flex items-start space-x-3 sm:space-x-4">
                    <div className={`p-2 sm:p-3 rounded-xl ${isActive ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}>
                      <Icon className="w-5 h-5 sm:w-6 sm:h-6" />
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center space-x-2 sm:space-x-3 mb-2">
                        <span className={`text-xs sm:text-sm font-semibold px-2 py-1 rounded-full ${isActive ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}>
                          {index + 1}
                        </span>
                        <h3 className={`text-base sm:text-lg font-semibold ${isActive ? 'text-primary' : 'text-foreground'}`}>
                          {step.title}
                        </h3>
                      </div>
          ...
          <div className="relative mt-8 lg:mt-0">
            <div className="sticky top-8">
              <Card className="p-4 sm:p-8 bg-gradient-to-br from-primary/10 to-accent/10 border-none shadow-xl px-[14px] py-[31px] mx-0">
                <div className="aspect-video bg-muted rounded-xl overflow-hidden relative">
                  {/* Placeholder for video - you can replace with actual video element */}
                  <img src={steps[activeStep].video} alt={`${steps[activeStep].title} demonstration`} className="w-full h-full object-cover" />
